function Update-EveryModule {
    ...
    $CurrentModules | ForEach-Object {
        ...
        Try {
            ...
            Write-Error "Module $($_.Name)$_"
            $GalleryModule = $null
        }
        if ($GalleryModule.Version -gt $_.Version) {
            if ($SkipMajorVersion -and $GalleryModule.Version.Split('.')[0] -gt $_.Version.Split('.')[0]) {
                Write-Warning "Skipping major version update for module $($_.Name)$($GalleryModule.Version)$($_.Version)"
            } else {
                Write-Verbose "$($_.Name)$($GalleryModule.Version)$($_.Version)"
                try {
                ...
                if ($KeepOldModuleVersions -ne $true) {
                    ...
                    if ($ExcludedModulesforRemoval -contains $_.Name) {
                        Write-Verbose "$($allversions.count)$($module.name) ]"
                        ...
                        try {
                            ...
                            Write-Error "Uninstalling old module $($_.Name)$_"
                        }
                    }
                }
            }
